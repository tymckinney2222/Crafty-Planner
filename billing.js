  ...
  async function syncOnLoad(){
    ...
    try{
      ...
      if(pro.purchaseToken){
        ...
        logStep('step 5b: ack result', JSON.stringify({ok: ackResult.ok, method: ackResult.method, needed: ackResult.needed, error: ackResult.error}));
        if(ackResult.ok && ackResult.needed){
          showDiagnosticToast('✅ Purchase acknowledged via ' + ackResult.method);
        }else if(ackResult.ok && !ackResult.needed){
          logStep('step 5c: subscription auto-acknowledged (v2 API, no explicit call needed)');
        }else{
          showDiagnosticToast('Ack failed: ' + (ackResult.error || 'unknown'));
        }
      }else{
