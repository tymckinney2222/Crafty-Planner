    ...
    try{
      ...
      if(pro.purchaseToken){
        ...
        for(const flavor of ['onetime', 'repeatable']){
          logStep('step 5b: trying ack with flavor=' + flavor);
          try{
            await svc.acknowledge(pro.purchaseToken, flavor);
            ackSucceeded = true;
            logStep('step 5c: ACK SUCCESS with ' + flavor);
            showDiagnosticToast('✅ Purchase acknowledged (' + flavor + ')');
            break;
          }catch(e){
            lastErr = e;
            logStep('step 5c: ack with ' + flavor + ' FAILED', (e && e.message) ? e.message : String(e));
          }
        }
      }else{
      ...
      if(!ackSucceeded && lastErr){
        logStep('all ack flavors failed');
