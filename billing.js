...
const CP_BILLING = (function(){
    ...
    try{
      ...
      try{
        if(activeProPurchase.purchaseToken){
          await svc.acknowledge(activeProPurchase.purchaseToken, 'repeatable');
        }
      }catch(ackErr){
  ...
  async function initiatePurchase(){
    ...
    try{
      ...
      if(!details || !details.length){
        console.error('[billing] product not found in Play Console:', PRODUCT_ID);
        showToast('❌ Subscription product not available', 'var(--red)');
        return false;
      }
      itemDetails = details[0];
    }catch(e){
      console.error('[billing] getDetails failed:', e);
      showToast('❌ Could not load subscription details', 'var(--red)');
      return false;
    }
    ...
    try{
      ...
      showToast('❌ Payment not available', 'var(--red)');
      return false;
    }
    ...
    try{
      ...
      showToast('❌ Purchase failed — please try again', 'var(--red)');
      return false;
    }
    ...
    // For subscriptions, use 'repeatable' — gets re-acknowledged each renewal.
    try{
      await svc.acknowledge(purchaseToken, 'repeatable');
    }catch(ackErr){
      console.error('[billing] acknowledgment FAILED — purchase will auto-refund:', ackErr);
      await response.complete('fail');
      showToast('❌ Purchase could not be confirmed', 'var(--red)');
      return false;
    }
  ...
  async function syncOnLoad(){
    try{
      // Skip if we checked recently and the user is already Pro — reduces
      // API load on repeated opens. Still re-check if they're currently free
      // (in case they just purchased on another device).
      let cached;
      try{cached = JSON.parse(localStorage.getItem('cp-profile') || '{}');}
      catch(e){cached = {};}
      const lastCheck = parseInt(localStorage.getItem(CACHE_KEY) || '0', 10);
      const age = Date.now() - lastCheck;
      if(cached.isPro && age < CACHE_MAX_AGE_MS){
        return; // cached Pro status is fresh enough
      }

      const status = await checkProStatus();
      // Only overwrite isPro in localStorage if we got a definitive answer
      // (available: true). If billing wasn't available at all, leave the
      // existing state alone — especially the dev toggle.
      if(status.available || status.reason === 'dev-toggle'){
        persistProStatus(status.isPro);
        if(typeof applyProfile === 'function')applyProfile();
      }
    }catch(e){
      console.warn('[billing] syncOnLoad silent failure:', e);
    }
  }
